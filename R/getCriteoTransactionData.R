#' @title Get Transactions Report Data
#' 
...
#' 
#' @export
getCriteoTransactionData <- function(URL, jobID){
  ...
  if(length(xmlData)==0){
    ...
    names(data)[names(data)=="transactionDateTime"] <- "date"
    #remove columns
    data$name <- NULL
    data$transactionDateTimePosix <- NULL
    #convert data types
    data[,1:ncol(data)] = sapply(data[,1:ncol(data)], as.character)
    data$date <- as.Date(data$date)
    try(data$orderValue = sapply(data$orderValue, as.numeric),silent=T)
    #arrange data
